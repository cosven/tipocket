...
func (c *TiKvDebugClients) CheckRaftStoreConsistency() {
	...
	isPeersEqual := func(p1, p2 []*metapb.Peer) bool {
		if (p1 == nil) != (p2 == nil) {
			return false
		}
		if len(p1) != len(p2) {
	...
	isRegionEpochEqual := func(e1, e2 *metapb.RegionEpoch) bool {
		if (e1 == nil) != (e2 == nil) {
			return false
		}
	...
	isRegionMetaEqual := func(r1 *metapb.Region, r2 *metapb.Region) bool {
		rangeEqual := bytes.Equal(r1.StartKey, r2.StartKey) && bytes.Equal(r1.EndKey, r2.EndKey)
		epochEqual := isRegionEpochEqual(r1.RegionEpoch, r2.RegionEpoch)
		peersEqual := isPeersEqual(r1.Peers, r2.Peers)
		return rangeEqual && epochEqual && peersEqual
	}
	...
	for url, debugClient := range c.debugClients {
		...
		for i := 0; i < len(response.States); i++ {
			...
			if !isRegionMetaEqual(collector.State.Region, currentState.Region) {
				log.Warnf("region meta not equal: origin: %v, current: %v",
					collector.State.Region, currentState.Region)
				continue
			}
			if collector.State.LeaderId != currentState.LeaderId {
				log.Warnf("region leader not equal: origin: %d, current: %d",
					collector.State.LeaderId, currentState.LeaderId)
				continue
			}
			if collector.State.LastIndex != currentState.LastIndex {
				log.Warnf("region LastIndex not equal: origin: %d, current: %d",
					collector.State.LastIndex, currentState.LastIndex)
				continue
			}
			if collector.State.AppliedIndex != currentState.AppliedIndex {
				log.Warnf("region AppliedIndex not equal: origin: %d, current: %d",
